    ...
    for zone_name, defaults in default_zone_defs.items():
        ...
        with col1:
            x = st.number_input(
                ...
                value=dx, step=0.01, format="%.4f"
            )
            w = st.number_input(
                f"{zone_name} W",
                key=f"{zone_name}_w",
                min_value=0.0, max_value=1.0,
                value=dw, step=0.01, format="%.4f"
            )
        with col2:
            y = st.number_input(
                f"{zone_name} Y",
                key=f"{zone_name}_y",
                min_value=0.0, max_value=1.0,
                value=dy, step=0.01, format="%.4f"
            )
            h = st.number_input(
                f"{zone_name} H",
                key=f"{zone_name}_h",
                min_value=0.0, max_value=1.0,
                value=dh, step=0.01, format="%.4f"
            )
...
# --- Ignore text rules (always visible) ---
...
if uploaded_file:
    ...
    for (bbox, text, prob) in results:
        ...
        if any(term in detected_text for term in st.session_state["ignore_terms"]):
            ...
            continue  # Skip infractions & zone checks
        ...
        for zone_name, (zx, zy, zw, zh) in abs_zones.items():
            if (tx >= zx and ty >= zy and
                (tx + tw) <= (zx + zw) and (ty + th) <= (zy + zh)):
                inside_any = True
